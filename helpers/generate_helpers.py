...
def save_images(
    ...
    # 💡 추가: 'all' 모드일 때 regression 그룹에 데이터 저장
    if image_reg is not None:
        # NetCDF4 라이브러리를 직접 사용
        f = writer.f # writer 객체가 가지고 있는 파일 핸들러
        reg_group = f.groups['regression']
        
        # (ensemble, y, x, channel) 형태로 변환
        reg_to_save = image_reg.permute(0, 2, 3, 1).numpy()
        ...
        for ens_idx in range(num_ensembles):
            for chan_idx, chan_meta in enumerate(output_channels_meta):
                data_slice = reg_to_save[ens_idx, :, :, chan_idx]
                # 파일에 쓰기
                reg_group[chan_meta.name][ens_idx, time_index] = data_slice
