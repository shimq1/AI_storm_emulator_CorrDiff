...
class RegressionLoss:
    ...
    def __call__(
        ...
        pad = 16
        # 마지막 두 채널만 패딩 제거
        D_yn_cropped = D_yn[..., pad:-pad, pad:-pad]
        y_cropped    = y[..., pad:-pad, pad:-pad]
        loss = weight * ((D_yn_cropped - y_cropped) ** 2)
...
class RegressionLossCE:
    ...
    def __call__(
        ...
        D_yn_cropped = D_yn[..., pad:-pad, pad:-pad]
        y_cropped    = y[..., pad:-pad, pad:-pad]
        loss1 = weight * (D_yn_cropped[:, scalar_channels] - y_cropped[:, scalar_channels]) ** 2
        loss2 = (
            weight
            * self.entropy(D_yn_cropped[:, self.prob_channels], y_cropped[:, self.prob_channels])[
                :, None
            ]
        )
