...
class NetCDFWriter:
    ...
    def __init__(
        self, f, lat, lon, input_channels, output_channels, has_lead_time=False, autoregressive_opt=False
    ):
        self._f = f
        self.has_lead_time = has_lead_time
        self.autoregressive_opt = autoregressive_opt
        # create unlimited dimensions
        f.createDimension("time")
        f.createDimension("ensemble")
        if self.autoregressive_opt:
            f.createDimension("step")
        ...
        for variable in output_channels:
            ...
            if not self.autoregressive_opt:
                ...
                self.truth_group.createVariable(
                    name, "f", dimensions=("time", "step", "y", "x")
                )
                self.prediction_group.createVariable(
                    name, "f",
                    dimensions=("ensemble", "time", "step", "y", "x")
                )
    ...
    def write_truth(self, channel_name, time_index, val, step_index=None):
        if self.autoregressive_opt:
            self.truth_group[channel_name][time_index, step_index, :, :] = val
        else:
            self.truth_group[channel_name][time_index, :, :] = val

    def write_prediction(self, channel_name, time_index, ensemble_index, val, step_index=None):
        if self.autoregressive_opt:
            self.prediction_group[channel_name][ensemble_index, time_index, step_index, :, :] = val
        else:
...
def get_time_from_range(times_range, time_format="%Y-%m-%dT%H:%M:%S"):
    ...
    interval = (
        datetime.timedelta(minutes=int(times_range[2]))
